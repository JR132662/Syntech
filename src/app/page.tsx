...
export default function Home() {
  ...
  return (
    <main className="pb-0">
      ...
      <section id="products" className="bg-syntech-offwhite px-4 py-16 sm:px-6 sm:py-24 lg:px-8 lg:py-28">
        <div className="mx-auto max-w-7xl">
          <div className="mb-12 flex flex-col gap-4 sm:mb-16 md:flex-row md:items-end md:justify-between">
            ...
            <Link
              href="#contact"
              className="shrink-0 text-sm font-semibold text-syntech-turf hover:underline"
            >
              Get a quote →
            </Link>
          </div>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4 lg:grid-cols-4">
            {categories.map((cat) => (
              <Link
                key={cat.title}
                href={cat.href}
                className="group relative aspect-[3/4] overflow-hidden rounded-xl bg-gray-200 sm:aspect-[4/5]"
              >
                <Image
                  src={cat.image}
                  alt={cat.title}
          ...
          <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4 lg:gap-12">
            {benefits.map((b) => (
              <div key={b.num} className="flex gap-4">
                <span className="font-mono text-2xl font-bold text-syntech-turf/80 sm:text-3xl">
                  {b.num}
                </span>
                <div>
                  <h3 className="font-display font-semibold text-syntech-black">{b.title}</h3>
                  <p className="mt-1 text-sm text-gray-500">{b.line}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Video */}
      <section className="bg-syntech-offwhite py-16 sm:py-24">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
          <h2 className="font-display text-xl font-bold tracking-tight text-syntech-black sm:text-2xl">
            See us in action
          </h2>
      ...
      <section className="border-t border-gray-200 bg-white py-16 sm:py-20 lg:py-24">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-4 sm:gap-6">
                {pressLogos.map((src) => (
                  <div
                    key={src}
                    className="relative flex min-h-0 min-w-0 items-center justify-center rounded-xl bg-gray-50 py-6 px-4 ring-1 ring-gray-100 transition hover:ring-syntech-turf/20 sm:py-8 sm:px-6"
                  >
      ...
      <section id="testimonials" className="border-t border-gray-200 bg-white py-16 sm:py-24 lg:py-28">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h2 className="font-display text-2xl font-bold tracking-tight text-syntech-black sm:text-3xl">
            What clients say
          </h2>
          <blockquote className="mt-8 max-w-3xl">
            <p className="font-display text-xl leading-relaxed text-syntech-black sm:text-2xl lg:text-3xl">
              &ldquo;{featuredQuote.quote}&rdquo;
            </p>
            <footer className="mt-6 text-sm font-semibold text-syntech-turf">
              — {featuredQuote.author}
            </footer>
          </blockquote>
          <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:mt-16 lg:grid-cols-4">
            {moreQuotes.map((t) => (
              <blockquote key={t.author} className="border-l-2 border-syntech-turf/30 pl-4">
                <p className="text-sm leading-relaxed text-gray-600">&ldquo;{t.quote}&rdquo;</p>
                <footer className="mt-3 text-xs font-semibold text-syntech-turf">— {t.author}</footer>
              </blockquote>
            ))}
          </div>
        </div>
      ...
      {/* Instagram strip */}
      <section className="border-t border-gray-200 bg-syntech-offwhite py-12 sm:py-16">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-6 px-4 sm:flex-row sm:px-6 lg:px-8">
          <div>
            <p className="font-display text-lg font-bold text-syntech-black sm:text-xl">
              Follow <span className="text-syntech-turf">@syntechturf</span>
            </p>
            <p className="mt-1 text-sm text-gray-500">Latest projects & tips</p>
          </div>
          <Link
      ...
      <section className="bg-syntech-turf py-14 sm:py-16">
        <div className="mx-auto max-w-2xl px-4 text-center sm:px-6 lg:px-8">
          ...
          <Link
            href="#contact"
            className="mt-6 inline-block rounded-full bg-white px-8 py-3.5 text-sm font-semibold text-syntech-turf transition hover:bg-gray-100"
          >
            Get your quote
          </Link>
        </div>
